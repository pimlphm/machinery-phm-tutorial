...
def plot_condition_normalization_comparison(
    ...
    for cond in df['condition_id'].unique():
        ...
        df_normalized.loc[mask, sensor_cols] = pd.DataFrame(scaled_values, index=df.loc[mask, sensor_cols].index, columns=sensor_cols)
        ...
        if subset in condition_normalize:
            ...
            for cond in np.unique(condition_ids):
                ...
                if cond in scalers[subset]['condition_scalers']:
                    ...
                    df_scaled.loc[mask, sensor_cols] = pd.DataFrame(scaled_values, index=df.loc[mask, sensor_cols].index, columns=sensor_cols)
            df = df_scaled
        else:
            scaler = scalers[subset]['standard_scaler']
            scaled_values = scaler.transform(df[sensor_cols])
            df.loc[:, sensor_cols] = pd.DataFrame(scaled_values, index=df.index, columns=sensor_cols)
