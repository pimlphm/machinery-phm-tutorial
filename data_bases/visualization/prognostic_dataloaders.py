...
def process_cmapss_data(base_path="/content/turbofan_data", 
    ...
    def wavelet_denoise(signal, wavelet='sym8', levels=4, mode='bayes', threshold_rule='median'):
        """
        Apply wavelet denoising using specified parameters
        
        Args:
            signal: Input signal array
            wavelet: Wavelet type ('sym8')
            levels: Decomposition levels (4)
            mode: Thresholding mode ('bayes')
            threshold_rule: Threshold rule ('median')
        """
        ...
        # Calculate threshold based on Bayes method with median rule
    ...
    def calculate_cumulative_mean_features(df, sensor_cols):
        """
        Calculate cumulative mean features to smooth fluctuations and preserve degradation trends
        """
        df_processed = df.copy()
        
        for sensor_col in sensor_cols:
            cumulative_col = f'{sensor_col}_cumavg'
            
            for engine_id in df['engine_id'].unique():
                engine_mask = df['engine_id'] == engine_id
                engine_data = df[engine_mask].sort_values('cycle')
    ...
    def calculate_monotonicity(df, sensor_cols):
        ...
        Monotonicity = mean(|#pos - #neg| / (n-1))
        where #pos is number of positive derivatives, #neg is number of negative derivatives
        """
        monotonicity_scores = {}
        ...
        for sensor_col in sensor_cols:
            ...
            for engine_id in df['engine_id'].unique():
                ...
                if len(signal) > 2:
                    ...
                    # Count positive and negative derivatives
                    n_pos = np.sum(derivatives > 0)
                    n_neg = np.sum(derivatives < 0)
                    n_total = len(derivatives)
                    
                    if n_total > 0:
                        # Calculate monotonicity score
                        monotonicity = abs(n_pos - n_neg) / n_total
                        engine_monotonicity.append(monotonicity)
            
            if engine_monotonicity:
                monotonicity_scores[sensor_col] = np.mean(engine_monotonicity)
            else:
                monotonicity_scores[sensor_col] = 0.0
    ...
    def create_windows_with_cumavg_features(data, window_size, stride, sensor_cols):
        """Create sliding windows using cumulative mean features"""
        X, y, window_indices = [], [], []
        
        for eid in data['engine_id'].unique():
            series = data[data['engine_id'] == eid].sort_values('cycle')
            s_vals = series[sensor_cols].values
    ...
    return {
        ...
        'processing_config': {
            ...
            'cumulative_mean_features': True,
            'z_score_normalization': True,
            'output_format': '[batch_size, time_steps, channels]'
        }
    }
